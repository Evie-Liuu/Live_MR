...
**Q: 開發模式啟動時噴 port 已被占用（`EADDRINUSE`／`bind: ... Only one usage of each socket address`）**
常見原因是前一次開發階段的行程沒有真的關閉（例如視窗直接叉掉而不是按 Ctrl+C）。確認 `7880`（dev LiveKit）、`3001`（backend dev）、`5173`（frontend dev）這幾個 port 有沒有殘留的舊行程還在跑（工作管理員找 `livekit-server.exe`／`node.exe`，或用 `netstat -ano | findstr :7880` 之類的指令查是哪個 PID 佔用），確認後手動結束該行程再重新啟動。
