...
- **學生加入延遲或重新整理**：若學生在老師按下「開始錄製」之後才加入房間，或在錄製進行中重新整理瀏覽器分頁，將無法收到錄製開始訊號。由於開始訊號不會被保存或重新播放，該次錄製將不會包含該位學生的音訊。老師端有「掛載時偵測進行中的錄製」的自動復原機制（`frontend/src/hooks/useRecording.ts` 中的 `getRecordings(roomId)` effect），但學生端目前尚未實作對應機制。
